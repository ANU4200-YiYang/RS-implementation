...
class RSModel(tez.model.Model):
    ...
    def forward(self, users, movies, ratings):
        ...
        if ratings is not None:
            ...
            cal_metrics = self.monitor_metrics(output, ratings.view(-1, 1))
...
def train():
    ...
    df_train, df_valid = model_selection.train_test_split(df, test_size=0.1, random_state=0, stratify=df.rating.values)
    ...
    model = RSModel(num_users=len(lbl_user.classes_), num_movies=len(lbl_movie.classes_))
