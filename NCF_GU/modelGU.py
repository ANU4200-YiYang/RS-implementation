...
class NCF(nn.Module):
    ...
    def forward(self, user,item):
        if not self.model == 'MLP':
            ...
            embed_user_GMF= torch.mean((self.embed_user_GMF(torch.LongTensor(user))), 0).repeat(self.item_num, 1)
            ...
            # print(embed_item_GMF.size())
            # torch.Size([3706, 32])
            # exit()
            output_GMF = embed_user_GMF * embed_item_GMF
        if not self.model == 'GMF':
            embed_user_MLP = torch.mean((self.embed_user_MLP(torch.LongTensor(user))), 0).repeat(self.item_num, 1)
            embed_item_MLP = self.embed_item_MLP(item)
            # print(embed_user_MLP.size())
            #torch.Size([3706, 128])
            # print(embed_item_MLP.size())
            #torch.Size([3706, 128])
        ...
        return prediction.view(-1)  # flatten(BCE)
